...
$(document).ready(function () {
  ...
  $('#displayName').keyup(function(key) {
    if (key.keyCode === 13) {
      Demo.Skylink.connect(Config.lobbyRoom, {
  ...
  $('#peerList').on('click', '.clientPeer', function () {
    Demo.Skylink.agentRequestCall($(this).attr('id'));
  });
});
//---------------------------------------------------
// Skyway
// Initialize all the Skyway settings
/*Demo.Skylink = new SkywayCC();
Demo.Skylink.init({
...
Demo.Skylink.on('peerJoined', function (peerId, peerInfo, isSelf) {
  ...
  if (isSelf) {
    ...
    if (peerInfo.call.peerType === Demo.Skylink.PEER_TYPE.CLIENT &&
      Demo.User.call.status === Demo.Skylink.CALL_READY_STATE.LOBBY) {
      Demo.User.userData.status = Demo.Events.watching;
      Demo.Skylink.startPeerEvent(Demo.User.userData, {
    ...
    // We should handle from SkylinkCC to prevent sending information
    // of agent
    if (Demo.Methods.getUserType() === Demo.Skylink.PEER_TYPE.AGENT &&
      peerInfo.call.peerType === Demo.Skylink.PEER_TYPE.CLIENT) {
      $('#peerList').append('<li>' +
...
Demo.Skylink.on('peerCallRequest', function (peerId, peerInfo, isSelf) {
  if (isSelf) {
    Demo.User = peerInfo;
  } else {
    Demo.Peers[peerId] = peerInfo;
    switch(peerInfo.call.status) {
    case Demo.Skylink.CALL_READY_STATE.REQUEST_CALL:
      var result = confirm(peerId + ' requested to call you. Accept?');
      Demo.Skylink.acceptRequestCall(peerId, result);
      break;
    case Demo.Skylink.CALL_READY_STATE.ACCEPTED_CALL:
      alert(peerId + ' has accepted your call.');
      Demo.Skylink.startRequestCall(peerId, function (userInfo, testInfo) {
        console.info(testInfo);
        Demo.Skylink.joinRoom(peerInfo.call.targetRoom, {
          audio: true,
          video: true
        });
      });
      break;
    case Demo.Skylink.CALL_READY_STATE.REJECTED_CALL:
      alert(peerId + ' has rejected your call.');
      break;
    case Demo.Skylink.CALL_READY_STATE.START_CALL:
      Demo.Skylink.startRequestCall(peerId, function (userInfo, testInfo) {
        console.info(testInfo);
        Demo.Skylink.joinRoom(peerInfo.call.targetRoom, {
          audio: true,
...
//---------------------------------------------------
// Demo custom events
Demo.Skylink.on(Demo.Events.watching, function (advertUrl) {
...
//---------------------------------------------------
Demo.Skylink.on(Demo.Events.onhold, function () {
